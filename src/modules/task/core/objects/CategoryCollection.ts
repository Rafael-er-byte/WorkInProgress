...
export default class CategoryCollection implements Collection {
  ...
  public addItem(category: Category): CategoryCollection {
    if (this.categories.length + 1 > this.limitOfCategories)
      throw new LimitExceeded('Categories limit exceeded');
    const exists = this.categories.find((c) => c.getId() === category.getId());
    if (exists)
      throw new ConflictDuplicateResource('This category already exists in this task', category);
