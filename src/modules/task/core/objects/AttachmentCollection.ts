...
export default class AttachmentCollection implements Collection {
  ...
  public addItem(attachment: Attachment): AttachmentCollection {
    if (this.attachments.length + 1 > this.limitOfattachments)
      throw new LimitExceeded('Attachment limit exeeded');
    if (attachment.getSize().getValue() > this.limitAttachmentMBSize)
      throw new LimitExceeded('Attachment too large');
    const exists = this.attachments.find(
      (savedattachment) => savedattachment.getUrl() === attachment.getUrl(),
    );
  ...
  public deleteItem(attachment: Attachment): AttachmentCollection {
    ...
    return new AttachmentCollection(
      this.attachments.filter((attach) => attach.getUrl() !== attachment.getUrl()),
    );
  }
