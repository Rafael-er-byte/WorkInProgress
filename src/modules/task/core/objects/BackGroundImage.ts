...
export default class BackGroundImage {
  ...
  constructor(attachment: Attachment) {
    if (attachment.getType() !== AllowedAttachents.image)
      throw new InvalidParameters('The resource must be an image');
    if (attachment.getSize().getValue() > this.limitImageSize)
      throw new LimitExceeded(`The image size must be less or iqual than ${this.limitImageSize}`);
