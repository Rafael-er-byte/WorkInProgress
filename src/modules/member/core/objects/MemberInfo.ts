...
export default class MemberInfo extends ValueObject {
  ...
  constructor(username: string, url: Attachment) {
    ...
    if (url.getSize().getValue() > this.maxAtachmentMbSize)
      throw new MemberProfileImageIsTooLarge(url);
    if (url.getType() !== this.attachmentType)
      throw new InvalidParameters('The attachment must be an image');
